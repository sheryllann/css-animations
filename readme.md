...
![Separation of Concerns](img/Separation-of-Concerns.png)
...
In the above image, the left and right-most drawings are **Keyframes** - they define the starting or ending point of a smooth transition. The drawings between the left and right-most image are **Inbetweens** - they don't have to be drawn on a storyboard, because the animator can assume what they will look like without specific visual instructions.

In web-based animations, Keframes work the same way - they represent the begining or ending state of the element being animated. However, our Inbetweens will be generated by code, instead of being filled in by hand later.
...
![Ease](img/ease.gif)
>From top to bottom: Ease in, Ease out, Ease
...
While we could go nuts and add all sorts of property changes in here, I would encourage you to consider Separation of Concerns when designing Keyframe Animations - they work best when they are clear, simple, and perform only one task. Our 'bounce' animation serves a single function - to bounce an element. By keeping it style-neutral, we allow ourselves to use the animation on lots of different elements.
